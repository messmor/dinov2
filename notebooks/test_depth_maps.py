...
for model_size in backbone_archs.keys():
    backbone_name = f"dinov2_{backbone_archs[model_size]}"
    DINOV2_BASE_URL = "https://dl.fbaipublicfiles.com/dinov2"
    head_config_url = f"{DINOV2_BASE_URL}/{backbone_name}/{backbone_name}_{HEAD_DATASET}_{HEAD_TYPE}_config.py"
    head_checkpoint_url = f"{DINOV2_BASE_URL}/{backbone_name}/{backbone_name}_{HEAD_DATASET}_{HEAD_TYPE}_head.pth"

    cfg_str = load_config_from_url(head_config_url)
    cp_path = f"/home/inseer/engineering/inseer_ml_pipeline/src/Hand_Orientation/dinov2/weights/{backbone_name}_{HEAD_DATASET}_{HEAD_TYPE}.pth"
    # with open(save_path, "w") as file:
    #     file.write(cfg_str)
    cfg = mmcv.Config.fromstring(cfg_str, file_format=".py")
    model = create_depther(cfg,
                           backbone_model=backbone_model,
                           backbone_size=BACKBONE_SIZE,
                           head_type=HEAD_TYPE)
    print("model type", type(model))
    load_checkpoint(model, head_checkpoint_url, map_location="cpu")
    save_checkpoint(model, cp_path)
# model.eval()
# model.cuda()
#
# # Show Example Image
